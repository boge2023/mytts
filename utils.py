...
def _find_checkpoints(out_dir: Path, iteration: int = 0) -> List[str]:
  checkpoints = list(glob.glob(f"{out_dir}/D_[0-9]*.pth")) + list(glob.glob(f"{out_dir}/G_[0-9]*.pth"))
  pattern = re.compile(r"D_([0-9]+).pth|G_([0-9]+).pth")
  iter_checkpoints = []
  for c in checkpoints:
    ...
    checkpoint_id = filename.replace(".pth", "").split("_")[1]
    iter_checkpoints.append((int(checkpoint_id), c))
...
def remove_checkpoint(out_dir, topk: int, rank: int):
  """Remove checkpoints from the given directory.

    We assume that checkpoint filename has the form `D_xxx.pt` and `G_xxx.pt`
  ...
  if rank != 0:
    return
  checkpoints = _find_checkpoints(out_dir)
  ...
  # There are same number of D_xxx.pth and G_xxx.pth, so topk should be doubled
  topk *= 2 
